...
export const saveResult = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { wpm, accuracy, testId, timeTaken } = req.body;
    if (!testId) {
    ...
    if (!timeTaken) {
      throw new CustomError("timeTaken is required", 404);
    }
    ...
    const newResult = new Result({ wpm, accuracy, timeTaken, testId, userId: req.user._id }) as IResult;
